...
class _ViewPaymentsScreenState extends State<ViewPaymentsScreen> {
              ...
              for (var key in bookingsMap.keys) {
                ...
                if (bookingData.containsKey('transactionCode') &&
                  ...
                  if (userSnapshot.exists) {
                    ...
                    bookingData['userToken'] = userData['token']; // Store user's FCM token
                  }
  ...
  Future<void> _updateBookingStatus(
    ...
    String title = newStatus == 'confirmed' ? 'Payment Confirmed' : 'Payment Rejected';
    String body = newStatus == 'confirmed' ? 'Your payment has been confirmed.' : 'Your payment has been rejected. Reason: $rejectionReason';
    String userToken = _payments.firstWhere((payment) => payment['bookingId'] == bookingId)['userToken'];
    _sendNotificationToUser(userToken, title, body);

    _fetchAllPayments();
  }

  void _sendNotificationToUser(String token, String title, String body) async {
